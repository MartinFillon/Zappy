...
class RessourcesManager {
  public:
    ...
    void init()
    {
        _map[{0, 0}] = Model3D(S1); //replace with food model
        _map[{0, 1}] = Model3D(S1);
        _map[{0, 2}] = Model3D(S2);
        _map[{0, 3}] = Model3D(S3);
        _map[{0, 4}] = Model3D(S4);
        _map[{0, 5}] = Model3D(S5);
        _map[{0, 6}] = Model3D(S6);
        _map[{1, 0}] = Model3D(M1); //replace with food model
        _map[{1, 1}] = Model3D(M1);
        _map[{1, 2}] = Model3D(M2);
        _map[{1, 3}] = Model3D(M3);
        _map[{1, 4}] = Model3D(M4);
        _map[{1, 5}] = Model3D(M5);
        _map[{1, 6}] = Model3D(M6);
        _map[{2, 0}] = Model3D(L1); //replace with food model
        _map[{2, 1}] = Model3D(L1);
        _map[{2, 2}] = Model3D(L2);
        _map[{2, 3}] = Model3D(L3);
        _map[{2, 4}] = Model3D(L4);
    ...
    void Draw(int size, int type, float x, float z, float tileSize = 1.f)
    {
        ...
        _map[{size, type}].Draw(
            {x, y, z},
            {tileSize / 10, scale, tileSize / 10},
            {0, (static_cast<int>(x) % 2) ? 1.f : -1.f, 0},
            (x + z) * 7
        );
    }
