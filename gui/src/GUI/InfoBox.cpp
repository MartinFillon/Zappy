...
void InfoBox::resize(int x, int y, int width, int height)
{
    m_x = x;
    m_y = y;
    m_width = width;
...
void InfoBox::display() const
{
    if (m_isPrint && m_item != nullptr) {
        Raylib::drawRectangle(m_x, m_y, m_width, m_height, (Color){0, 0, 0, 200});
        auto infoList = m_item->getStringInfo();
        int lineCount = 0;

        for (auto line : infoList) {
            Raylib::drawText(line, m_x, m_y + (lineCount * m_lineHeight), m_lineHeight, WHITE);
            lineCount += 1;
        }
        lineCount += 2;
        for (std::size_t i = 0; i < m_itemName.size(); i++) {
            ...
            Raylib::drawText(line, m_x, m_y + ((lineCount + i) * m_lineHeight), m_lineHeight, WHITE);
        }
    }
}
