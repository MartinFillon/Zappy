...
bool ArgParser::get<bool>(const std::string &param) const
{
    ...
    if (value == "true" || value == "1") {
        ...
        throw ArgParserException("Invalid value for bool: " + value, "ArgParser::get<bool>");
    }
}
...
void ArgParser::checkMandatoryArgs(const std::set<std::string> &providedArgs) const
{
    for (const auto &arg : mandatoryArgs_) {
        if (providedArgs.find(arg) == providedArgs.end()) {
            throw ArgParserException("Mandatory parameter missing: " + arg, "ArgParser::checkMandatoryArgs");
        }
    }
}
            ...
            if (i + 1 < argc) {
                ...
                if (types_.find(key) != types_.end()) {
                    if (!isValidType(key, value)) {
                        throw ArgParserException("Invalid value for parameter: " + key, "ArgParser::parseArgs");
                    }
                    params_[key] = value;
                    providedArgs.insert(key);
                } else {
                    throw ArgParserException("Unknown parameter: " + key, "ArgParser::parseArgs");
                }
            } else {
                throw ArgParserException("Missing value for parameter: " + key, "ArgParser::parseArgs");
            }
        } else {
            throw ArgParserException("Invalid parameter format: " + param, "ArgParser::parseArgs");
        }
    }
